...
resource "dynatrace_metric_events" "team_dynamodb_server_error" {
  ...
  query_definition {
    type            = "METRIC_SELECTOR"
    metric_selector = "cloud.aws.dynamodb.systemErrorsByAccountIdOperationRegionTableName:sum:filter(or(eq(\"aws.account.id\",${var.team_account_id}),contains(\"tablename\",\"-main\"))):splitBy(\"tablename\")"
  }
}
